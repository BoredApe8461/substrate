...
You can start a local Substrate development chain with running `substrate run --dev`.
...
[source, shell]
----
substrate run --chain ~/mychain.json
----
...
[source, shell]
----
substrate run --chain ~/mychain.json --validator --key ...
----
...
Detailed logs may be shown by running the node with the following environment variables set: `RUST_LOG=debug RUST_BACKTRACE=1 cargo run -- run --dev`.
...
[source, shell]
cargo run --release -- run --name my_custom_name
