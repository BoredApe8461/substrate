...
// Global Singleton to keep track of the FunctionExecutor instance.
lazy_static! {
    static ref thisFE: Option<
        Box<
            Mutex<
                FunctionExecutor<'static, Externalities<Blake2Hasher>>
            >
        >
    > = None;
}
...
#[no_mangle] extern fn Z_envZ_ext_print_hexZ_vii(data: *const u8, len: u32) {
...
#[no_mangle] extern fn Z_envZ_ext_malloccZ_ii(size: usize) -> *mut u8 {
    let r = thisFE.lock().unwrap().heap.allocate(size);
    debug_trace!(target: "sr-io", "malloc {} bytes at {}", size, r);
    Ok(r)
}

#[no_mangle] extern fn Z_envZ_ext_freeZ_v(addr: *mut u8) {
    thisFE.lock().unwrap().heap.deallocate(addr);
    debug_trace!(target: "sr-io", "free {}", addr);
    Ok(())
}

#[no_mangle] extern fn Z_envZ_ext_set_storageZ_viiii(key_data: *const u8, key_len: u32, value_data: *const u8, value_len: u32) {
...
#[no_mangle] extern fn Z_envZ_ext_clear_storageZ_vii(key_data: *const u8, key_len: u32) {
...
#[no_mangle] extern fn Z_envZ_ext_exists_storageZ_iii(key_data: *const u8, key_len: u32) -> u32 {
...
#[no_mangle] extern fn Z_envZ_ext_clear_prefix(prefix_data: *const u8, prefix_len: u32) {
...
#[no_mangle] extern fn Z_envZ_ext_get_allocated_storage(key_data: *const u8, key_len: u32, written_out: *mut u32) -> *mut u8 {
...
#[no_mangle] extern fn Z_envZ_ext_get_storage_intoZ_iiiiii(key_data: *const u8, key_len: u32, value_data: *mut u8, value_len: u32, value_offset: u32) -> u32 {
...
#[no_mangle] extern fn Z_envZ_ext_storage_rootZ_vi(result: *mut u8) {
...
#[no_mangle] extern fn Z_envZ_ext_storage_changes_rootZ_iiiji(parent_hash_data: *const u8, parent_hash_len: u32, parent_number: u64, result: *mut u8) -> u32 {
    ...
    let raw_parent_hash = thisFE.lock().unwrap().memory.get(parent_hash_data, parent_hash_len as usize)
    ...
    let r = thisFE.lock().unwrap().ext.storage_changes_root(parent_hash, parent_number);
    if let Some(ref r) = r {
        thisFE.lock().unwrap().memory.set(result, &r[..]).map_err(|_| UserError("Invalid attempt to set memory in ext_storage_changes_root"))?;
    }
    Ok(if r.is_some() { 1u32 } else { 0u32 })
}

#[no_mangle] extern fn Z_envZ_ext_blake2_256_enumerated_trie_root(values_data: *const u8, lens_data: *const u32, lens_len: u32, result: *mut u8) {
...
#[no_mangle] extern fn Z_envZ_ext_twox_128Z_viii(data: *const u8, len: u32, out: *mut u8) {
    let result = if len == 0 {
...
#[no_mangle] extern fn Z_envZ_ext_twox_256(data: *const u8, len: u32, out: *mut u8)  {
    let result = if len == 0 {
        twox_256(&[0u8; 0])
    } else {
...
#[no_mangle] extern fn Z_envZ_ext_blake2_256Z_vii(data: *const u8, len: u32, out: *mut u8) {
    let result = if len == 0 {
        blake2_256(&[0u8; 0])
    } else {
...
#[no_mangle] extern fn Z_envZ_ext_ed25519_verifyZ_iiiii(msg_data: *const u8, msg_len: u32, sig_data: *const u8, pubkey_data: *const u8) -> u32 {
    let mut sig = [0u8; 64];
...
#[no_mangle] extern fn Z_envZ_ext_sandbox_invokeZ_iiiiiiiii(instance_idx: u32, export_ptr: *const u8, export_len: usize, args_ptr: *const u8, args_len: usize, return_val_ptr: *const u8, return_val_len: usize, state: usize) -> u32 {
