...
use consensus::{BlockImport, ImportOracle, Error as ConsensusError};
	...
	RA: Send + Sync
{
	fn import(&self, block: ImportBlock<Block>, new_authorities: Option<Vec<AuthorityIdFor<Block>>>)
		-> Result<ImportResult, ConsensusError>
	{
		(self as &SubstrateClient<B, E, Block, RA>).begin(|operation| {
			(self as &SubstrateClient<B, E, Block, RA>).import_block(operation, block, new_authorities)
		})
	}
